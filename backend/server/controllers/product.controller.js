...
exports.createProduct = async (req, res) => {
  try {
    const { vaultId, name, brand, category, purchaseDate, purchasePrice,
            warrantyExpiry, serialNumber, tags, notes, billImageUrl, billPdfUrl } = req.body;

    if (!await isMember(vaultId, req.user._id))
      return res.status(403).json({ message: 'Access denied' });

    const product = await Product.create({
      vaultId, name, brand, category, purchaseDate, purchasePrice,
      warrantyExpiry, serialNumber, tags, notes, billImageUrl, billPdfUrl,
...
exports.getProducts = async (req, res) => {
  try {
    ...
    const sortMap = {
      ...
      'price-high': { purchasePrice: -1 }, 'price-low': { purchasePrice: 1 },
      name: { name: 1 }, warranty: { warrantyExpiry: 1 },
    };
