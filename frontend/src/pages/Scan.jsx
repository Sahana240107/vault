...
const Scan = () => {
  ...
  const [ocrItems,    setOcrItems]    = useState([]);   // all extracted products
  const [selectedIdx, setSelectedIdx] = useState(null); // which item is picked
  ...
  // Fill the form from a selected OCR item
    ...
    try {
      ...
      if (items.length > 0) {
        ...
        if (items.length === 1) {
          ...
          // Multiple products — show picker, don't auto-fill yet
          setSelectedIdx(null);
        }
      }
      setOcrStatus('done');
    } catch (err) { setOcrStatus('error'); setError('Upload failed: ' + err.message); }
  };
  ...
  const handleWarrantyFile = async (file) => {
    ...
    if (file.type.startsWith('image/')) {
      const r = new FileReader(); r.onload = e => setWarrantyPreview(e.target.result); r.readAsDataURL(file);
    } else setWarrantyPreview('');
    try {
      const data = await uploadBill(file);
      setWarrantyUrl(data.url);
      if (data.ocr?.warrantyExpiry) { setField('warrantyExpiry', data.ocr.warrantyExpiry); setWarrantySrc('card-ocr'); }
      setWarrantyStatus('done');
    } catch (err) { setWarrantyStatus('error'); setError('Warranty card upload failed: ' + err.message); }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let vaultId;
      if (vaults.length === 0) { await api('/vaults', 'POST', { name: 'My Vault' }); vaultId = (await api('/vaults/my'))[0]._id; }
      else vaultId = vaults[0]._id;
  ...
  const handleReset = () => {
    setForm(EMPTY_FORM); setBillUrl(''); setBillType(''); setPreview(''); setOcrStatus('idle');
    setOcrItems([]); setSelectedIdx(null);
    setWarrantyUrl(''); setWarrantyPreview(''); setWarrantyStatus('idle'); setWarrantySrc(''); setError('');
  };

  const statusCard = {
    idle:      { border:'var(--cyan)',   bg:'var(--cyan-dim)',       icon:'🤖', color:'var(--cyan)',   title:'AI OCR Ready',               sub:'Extracts name, price, date, warranty', badge:<span className="annot" style={{marginLeft:'auto'}}>LIVE</span> },
    uploading: { border:'var(--gold)',   bg:'rgba(232,184,75,0.08)', icon:'⏳', color:'var(--gold)',   title:'Uploading & Running OCR…',   sub:'Reading your bill…', badge:null },
    ...
    error:     { border:'var(--danger)', bg:'rgba(245,75,75,0.08)',  icon:'❌', color:'var(--danger)', title:'Upload failed — fill manually', sub:'', badge:null },
  }[ocrStatus];

  const warrantyBadge = {
    idle:      { color:'var(--text-muted)', icon:'📋', text:'No warranty card uploaded yet' },
    uploading: { color:'var(--gold)',       icon:'⏳', text:'Uploading & scanning…' },
    done:      { color:'var(--cyan)',       icon:'✅', text: warrantySrc === 'card-ocr' ? 'Warranty date auto-filled ↑' : 'Card saved' },
    error:     { color:'var(--danger)',     icon:'❌', text:'Upload failed' },
  }[warrantyStatus];

  const warrantySrcChip = warrantySrc && (
    <span style={{ fontSize:10, fontWeight:700, letterSpacing:'0.06em', padding:'2px 8px', borderRadius:10, marginLeft:6,
      ...
      <main className="main-content">
        ...
        <div className="grid-2">
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <div className="scan-zone"
              style={{ position:'relative', height:260, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
                cursor:ocrStatus==='uploading'?'wait':'pointer', border:dragOver?'2px dashed var(--gold)':undefined, transition:'border 0.2s' }}
              ...
              {preview ? (
                ...
                <><span style={{ fontSize:48, marginBottom:12, display:'block' }}>{ocrStatus==='uploading'?'⏳':dragOver?'📥':'🧾'}</span>
                  <h3 style={{ fontFamily:'Syne,sans-serif', fontSize:18, fontWeight:700, marginBottom:6 }}>
                    {ocrStatus==='uploading'?'Processing…':dragOver?'Drop it!':'Drop Receipt Here'}</h3>
            ...
            {/* ── Multi-item picker (only shown when bill has 2+ products) ── */}
              ...
              <div className="card" style={{ borderColor:'var(--gold)', background:'rgba(232,184,75,0.06)' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {ocrItems.map((item, idx) => (
                    <button
                      key={idx}
                      onClick={() => { setSelectedIdx(idx); applyOcrItem(item); }}
                      style={{
                    ...
                    >
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:8 }}>
                        ...
                        <div style={{ fontSize:14, fontWeight:700, color:'var(--cyan)', flexShrink:0 }}>
                          {item.purchasePrice != null ? `₹${Number(item.purchasePrice).toLocaleString('en-IN')}` : ''}
                        </div>
                      </div>
                    </button>
                  ))}
                ...
                <p style={{ fontSize:11, color:'var(--text-muted)', marginTop:10 }}>
                  Each product must be saved separately. Select and save one at a time.
                </p>
              </div>
            )}
            ...
            <div className="card" style={{ borderColor:warrantyDragOver?'var(--gold)':warrantyStatus==='done'?'var(--cyan)':'var(--border)', transition:'border-color 0.2s' }}>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize:11, color:'var(--text-muted)', marginTop:2 }}>Separate document — OCR extracts expiry date</p>
                </div>
                {warrantyStatus==='done' && warrantyUrl && (
                  <a href={warrantyUrl} target="_blank" rel="noreferrer" style={{ marginLeft:'auto', fontSize:11, color:'var(--gold)', textDecoration:'underline' }}>View ↗</a>
                )}
              </div>
              <div onDragOver={e=>{e.preventDefault();setWarrantyDragOver(true);}} onDragLeave={e=>{e.preventDefault();setWarrantyDragOver(false);}}
                onDrop={e=>{e.preventDefault();setWarrantyDragOver(false);handleWarrantyFile(e.dataTransfer.files[0]);}}
                style={{ border:`1.5px dashed ${warrantyDragOver?'var(--gold)':'var(--border)'}`, borderRadius:10, padding:'14px 12px',
                  display:'flex', alignItems:'center', gap:14, background:warrantyDragOver?'rgba(232,184,75,0.06)':'var(--surface)',
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:12, fontWeight:600, color:warrantyBadge.color, marginBottom:2 }}>{warrantyBadge.icon} {warrantyBadge.text}</div>
                  <div style={{ fontSize:11, color:'var(--text-muted)' }}>
                    {warrantyStatus==='idle'&&'Drag & drop or Browse · JPG, PNG, PDF'}
                    {warrantyStatus==='uploading'&&'Scanning…'}
                  ...
                  <div style={{ display:'flex', gap:6, flexShrink:0 }}>
                    ...
                    {warrantyStatus==='done' && (
                      <button className="btn-ghost" style={{ fontSize:11, padding:'6px 10px', color:'var(--danger)' }}
                        onClick={()=>{setWarrantyUrl('');setWarrantyPreview('');setWarrantyStatus('idle');setWarrantySrc('');}}>✕</button>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="card-glint">
            ...
            <div className="grid-2">
              ...
              <div className="form-group">
                ...
                <select className="input-field" value={form.category} onChange={e=>setField('category',e.target.value)}>
                  <option>Electronics</option><option>Appliance</option><option>Vehicle</option><option>Furniture</option><option>Other</option>
                </select>
              </div>
            </div>
            <div className="grid-2">
            ...
            <div className="grid-2">
              <div className="form-group">
                <div className="input-label" style={{ display:'flex', alignItems:'center' }}>WARRANTY EXPIRY {warrantySrcChip}</div>
                <input className="input-field" type="date" value={form.warrantyExpiry}
                  ...
                  style={{ borderColor:form.warrantyExpiry?(warrantySrc==='card-ocr'?'var(--cyan)':warrantySrc==='bill-ocr'?'var(--gold)':'var(--border)'):'var(--border)', transition:'border-color 0.3s' }} />
                {form.warrantyExpiry && warrantySrc && (
                  <div style={{ fontSize:10, color:'var(--text-muted)', marginTop:4 }}>
                    {warrantySrc==='card-ocr'&&'🛡️ From warranty card'}{warrantySrc==='bill-ocr'&&'🧾 From bill scan'}{warrantySrc==='manual'&&'✏️ Entered manually'}
                  </div>
                )}
              </div>
              <div className="form-group">
            ...
            {warrantyStatus==='done' && warrantyPreview && (
              <div style={{ display:'flex', alignItems:'center', gap:10, background:'rgba(99,202,183,0.06)', border:'1px solid rgba(99,202,183,0.2)', borderRadius:8, padding:'10px 14px', marginBottom:12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:11, color:'var(--text-muted)' }}>Saved · {warrantyUrl && <a href={warrantyUrl} target="_blank" rel="noreferrer" style={{ color:'var(--gold)' }}>View ↗</a>}</div>
                </div>
              </div>
            )}
            ...
            <div className="form-group">
              ...
              <textarea className="input-field" style={{ height:60, resize:'none' }} placeholder="Any additional notes…" value={form.notes} onChange={e=>setField('notes',e.target.value)} />
            </div>
